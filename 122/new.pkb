  ...
  /**
   * Валидация и парсинг входного JSON для извлечения XML-квитанции.
   */
  ...
  IS
    v_xml_content CLOB;
    v_log_prefix  VARCHAR2(256) := 'AddReceiptInfoReport AddReceiptInfo_RC_ReportRun traceId=''' || COALESCE(p_trace_id_input, 'NULL') || '''';
    v_json_obj    JSON_OBJECT_T;
    v_receipt_value JSON_ELEMENT_T;
BEGIN
    IF p_json_input IS NULL OR p_json_input = '{}' OR p_json_input = '[]' THEN
      it_log.log(v_log_prefix || ' Empty or trivial input', it_log.C_MSG_TYPE__DEBUG);
      RAISE_APPLICATION_ERROR(-20002, 'Empty input JSON');
END IF;
...
SELECT JSON_OBJECT(
               ...
               'lim_date'    VALUE CASE WHEN p_limit_date IS NOT NULL THEN TO_CHAR(p_limit_date, 'DD.MM.YYYY') ELSE '' END,
               'send_day'    VALUE CASE WHEN p_send_day IS NOT NULL THEN TO_CHAR(p_send_day, 'DD.MM.YYYY') ELSE '' END,
               'reg_day'     VALUE CASE WHEN p_reg_day IS NOT NULL THEN TO_CHAR(p_reg_day, 'DD.MM.YYYY') ELSE '' END,
...
END IF;
    ...
    DECLARE v_json_clob CLOB; BEGIN
      v_json_clob := BuildReceiptJson(v_form_code, v_limit_date_db, v_send_time, v_reg_time, v_status, v_message_detail);
...
BEGIN
      v_xml_content := ParseReceiptXml(p_json_input, p_trace_id_input);
...
WHEN OTHERS THEN
        ...
        v_errors_array.append(GetErrorObjAndLog(p_trace_id_input, C_ERR_99_CODE, 'Ошибка при обработке квитанции: ' || SQLERRM));
        v_json_output := BuildJsonOutput(p_errors_array => v_errors_array);
        it_log.log(v_log_prefix || ' Returning (processing error): ' || DBMS_LOB.SUBSTR(v_json_output, 2000, 1), it_log.C_MSG_TYPE__DEBUG);
RETURN v_json_output;
END;
